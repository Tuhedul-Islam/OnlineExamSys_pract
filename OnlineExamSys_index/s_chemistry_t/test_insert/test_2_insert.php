...
for($j=1;$j<=30;$j++){
	...
	if($flag2 == 1 && $flag1 == 1 && $flag[$j][1] == 1 && $flag[$j][2] == 1 && $flag[$j][3] == 1 && $flag[$j][4] == 1){
		...
		$sql=mysqli_query($conn,"INSERT INTO `ch_ques_2`(`ques_nm`, `optn_1`, `optn_2`, `optn_3`, `optn_4`,`answer`) VALUES ('$ques[$j]','".$option[$j][1]."','".$option[$j][2]."','".$option[$j][3]."','".$option[$j][4]."','$answer[$j]')");	
	}
}
